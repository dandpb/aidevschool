...
# why: suffix fallback walk must skip vendored/heavy dirs like the inventory walk
...
_SUFFIX_INDEX: dict[str, list[Path]] | None = None


def _suffix_lookup(root: Path, cite_norm: str) -> Path | None:
    """Repo-wide suffix match for monorepo package-relative cites (src/x.ts → engines/p/src/x.ts)."""
    ...
    if _SUFFIX_INDEX is None:
        ...
        _SUFFIX_INDEX = idx
    hits = _SUFFIX_INDEX.get(Path(cite_norm).name) or []
    for h in hits:
        try:
            rp = h.resolve().relative_to(root.resolve()).as_posix()
        except (ValueError, OSError):
            continue
        if rp == cite_norm or rp.endswith("/" + cite_norm):
            return h
    return None
...
def foreign_skill_anchor_near_cite(root: Path, text: str, cite: str) -> bool:
    """True when the cite's paragraph mentions a skill that has no local tree.

    why: stack-agnostic teaching prose says "load the `dev` skill and read
    `references/view.md`" — when that anchor skill is absent from this repo the
    cite is an example, not a claim about this harness.
    """
    for m in re.finditer(re.escape(cite), text):
        before = text.rfind("\n\n", 0, m.start())
        after = text.find("\n\n", m.end())
        ...
        para = text[start:end]
        for name in mentioned_skill_names(para):
            if "/" in name:
                continue
            local = any(
                (root / tree / name).is_dir()
...
def is_placeholder_cite(cite: str) -> bool:
    if "*" in cite or "<" in cite or cite.endswith("/"):
        return True
    # why: "lib/...", ".agents/…" are pedagogical shorthands, never real paths
...
def resolve_cite(
    ...
    for c in candidates:
        ...
        try:
            ...
            continue
    # why: directory cites (`.agents/skills`, `evidence/<run>`) are real surfaces too
    for c in candidates:
        try:
            if c.is_dir():
                return c.resolve(), tried
            ...
            continue
    # why: monorepos cite package-relative paths (src/… inside engines/…)
    if "/" in cite_norm:
        hit = _suffix_lookup(root, cite_norm)
        if hit is not None:
            return hit.resolve(), tried + [f"suffix:{hit}"]
...
def check_file(root: Path, source: Path, commands: set[str], finding_id: list[int]) -> list[Finding]:
    ...
    for cite in extract_surface_cites(text):
        ...
        if not is_concrete_checkable_cite(cite):
            continue
        # why: "load the `dev` skill and read references/…" in stack-agnostic
        # teaching prose targets a skill this repo doesn't have — not BROKEN
        if normalize_cite(cite).startswith("references/") and foreign_skill_anchor_near_cite(
            root, text, cite
        ):
            continue
...
def _command_ok(token: str, kind: str, commands: set[str]) -> bool:
    if not token or is_placeholder_cite(token) or "<" in token:
        return True
    if token.startswith("-"):
        # why: `pnpm --filter pkg dev` — flags are not script names
        return True
