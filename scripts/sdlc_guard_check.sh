...
run_checks() {
  ...
  if pr_context; then
    ...
    case "$pr_author_login" in
      ...
      # '/', so the prefix is unambiguous (AID-2476: the gh shape alone used
...
$big_filler"
  ...
  # AID-2476 regression: the REAL gh CLI renders app authors as
  # {"is_bot":true,"login":"app/<slug>"} — no __typename, no [bot] suffix
  ...
  # AID-2476: gh-shape author ({"is_bot":true,"login":"app/<slug>"}) must
  # trip Stage 2 — trigger without a citation fails closed, and the same
  # shape with a valid pre-merge citation passes.
  pr2_scenario "$T2/bot_gh_open" "gh-shape bot author triggers Stage 2 without citation (AID-2476)" 1 "gh bot change" -- \
    "printf 'gh bot change\n' >> src/app.py"
  pr2_scenario "$T2/bot_gh_cited_premerge" "gh-shape bot author with pre-merge citation passes (AID-2476)" 0 "gh bot cited" -- \
    "printf 'gh bot change\n' >> src/app.py"
