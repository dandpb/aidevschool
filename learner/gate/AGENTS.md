...
| Independent literacy verifier (standalone site) | `netlify-functions/literacy-verify.mjs`, `netlify-functions/README.md` | Fixed l02-v3 contract verifier called by LiteracyDojo via `VITE_LITERACY_VERIFIER_URL`; recomputes from the structured answer, fails closed outside the pinned activity, never trusts producer claims. Contract test `tests/literacy_verify_netlify.test.mjs` runs in CI. Tracked from stash-only provenance per AID-941. |
