...
from learner.gate.core import gap_days, parse_iso
...
def expected_attempt_id(cid: str, attempt_no: int) -> str:
    """Return the script-owned id that also seeds the deterministic G3 draw."""
...
def primary_pass_count(
    ...
    served = (
        "G3" if primary["gate_id"] == "G4" and not llm_enabled else primary["gate_id"]
    )
    if served == "G3":
    ...
    for event in ledger:
        ...
        if (
            ...
            continue
        if primary["gate_id"] == "G4":
            if payload["evidence"]["verifier"].get("rubric_id") == primary_rubric:
                return 1
        else:
            return 1
    return 0
