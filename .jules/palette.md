...
## 2025-07-12 - Adding context to repeated buttons
**Learning:** Found that repeated "Abrir briefing" buttons in the roadmap view lacked context for screen readers. They would just announce "Abrir briefing, button" repeatedly.
**Action:** When rendering repeated buttons or links in lists and grids, always include a descriptive `aria-label` containing the specific item's context (such as the item title) to disambiguate the action for screen readers.
