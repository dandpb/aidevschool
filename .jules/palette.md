...
## 2026-09-10 - Synchronizing ARIA Expanded State on Toggle Buttons
**Learning:** When adding `aria-expanded` to a toggle button that controls a collapsible panel (like a configuration menu), it's crucial to update the `aria-expanded` state not just when the button itself is clicked, but also when the panel is shown or hidden programmatically by other actions (e.g., clicking a 'Save' button inside the panel, or a validation logic automatically showing the panel). Leaving these states disconnected causes the screen reader's context to desync from the visual UI state.
**Action:** Always identify all code paths that alter the visibility of the `aria-controls` target panel, and proactively sync the `aria-expanded` attribute on the corresponding toggle button during those transitions.
