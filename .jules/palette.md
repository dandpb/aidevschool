...
## 2026-07-27 - Accessible Streak Display
**Learning:** When applying `aria-label` to generic structural elements (like `<strong>` or `<span>`) to hide purely visual text (via `aria-hidden="true"`), always include a semantic role (e.g., `role="text"\' or `role="group"`) to ensure screen readers do not ignore the label.
**Action:** Apply `role="text"` alongside `aria-label` when overriding the accessible name of inline elements that contain visual-only symbols or emojis.
