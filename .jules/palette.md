...
## 2026-07-17 - Unique ARIA labels for repeated grid/list buttons
...
## 2026-07-19 - Hide decorative text and fix SVG semantics
**Learning:** Decorative text initials (like "CD" for codexDojo) and purely visual numbering prefixes in buttons create noise and stuttering for screen reader users. Additionally, SVGs used as images must have a clear `role="img"`.
**Action:** Add `aria-hidden="true"` to spans that are purely visual decorations or redundant information within an element, and add `role="img"` to informative SVGs.
