...
## 2024-05-18 - Replacing sort and intermediate array with O(N) scan in pathfinding
**Learning:** `Array.prototype.sort()` coupled with `Array.prototype.findIndex()` inside the comparator callback is incredibly slow for small arrays compared to a single-pass O(N) linear scan, and intermediate array allocations (`[].push()`) within hot loops (like a game engine's pathfinder) create substantial GC pressure.
**Action:** In deterministic simulations (like `miniTown`), when finding the "best" candidate among a small set of neighbors, use a linear O(N) scan tracking the minimum score instead of `.push()` + `.sort()`. When mimicking stable sorting to preserve determinism, use strict inequality (`<`) so the first candidate with a winning score is retained. Ensure to handle cases where the best candidate is still null (e.g., `if (bestCandidate === null || score < bestScore)`).
