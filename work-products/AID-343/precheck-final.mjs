// AID-343 pre-check (adapted from _work-products/AID-305/precheck-bridge.mjs)
// Expected pin: 6d72735ba2113cb6198c4d7be26c2f01e5f5d694 (PR #182 merge, l01-l14 batch)
...
const OS_SHA = '49d7cc8a3fba4b3e3c4ddbc1d3708cf5657123357d8f53e11321effa37320686'
const results = []
...
check('os-pins-pixelpixelDojo-immutable', osJs.includes('https://6a920159a8d5e2dfdd7fbeca--singular-crostata-273e7e.netlify.app/'), 'VITE_PIXELDOJO_URL must be the immutable deploy, not a dev fallback')
...
//    .voxel-world reflows with min-width:0
...
// 6. live MOTOR handshake: batch endpoints (l01 first, l02 cohort, l04 first-new, l14 last) + Dev regression
const browser = await chromium.launch()
for (const [label, path] of [['l01', '/mission/ai-pratica/l01'], ['l02', '/mission/ai-pratica/l02'], ['l04', '/mission/ai-pratica/l04'], ['l14', '/mission/ai-pratica/l14'], ['warehouse', '/mission/dev/game-02-warehouse']]) {
...
// 7. reflow predicate inside the embedded iframe at 320px AND ~298px (AID-271/AID-264 §3)
...
// 8. verification bridge: 200 same-origin JSON token; 403 JSON without Sec-Fetch-Site
{
...
// 9. chapter-map h1 counts the launchable catalog at runtime (14 IA Prática + 3 Dev = 17)
{
  const page = await browser.newPage({ viewport: { width: 1280, height: 800 } })
  try {
    await page.goto(BASE + '/', { waitUntil: 'domcontentloaded' })
    ...
    check('os-runtime-copy-17-missions', h1 === '17 missões, uma sequência', JSON.stringify(h1))
  } catch (e) {
    check('os-runtime-copy-17-missions', false, e.message.slice(0, 120))
  }
  await page.close()
}
await browser.close()
