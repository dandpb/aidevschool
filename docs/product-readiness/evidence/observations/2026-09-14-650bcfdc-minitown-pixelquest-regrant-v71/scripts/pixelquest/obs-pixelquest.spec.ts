...
// QA Lead AID-1924 — walk de observação independente pixel-quest (re-grant
// v71, PR #427, árvore 650bcfdc). Cobre as assertions "observation" dos 3
// cenários do use case pixelquest-evidence-encounter:
...
// Producer gates (pnpm run smoke: 5 specs) são fato separado deste walk.
...
// Lab 01 — Rate Limiter (token_bucket): mesma timeline determinística do smoke
// (12 ações) — o replay precisa re-emitir o mesmo núcleo determinístico.
...
test("QA walk: encounter evidence, anti-mastery copy, returning replay, recovery", async ({ page }) => {
  ...
  await page.screenshot({ path: `${BUNDLE}/ev/qapx71-01-encounter-evidence.png` });
  ...
  // (sem ressurreição) — nenhum registro acessível após reload (array vazio ou
  // hook ainda ausente até o próximo encontro; ambos = 0 registros).
  ...
  await page.screenshot({ path: `${BUNDLE}/ev/qapx71-02-returning-replay.png` });
