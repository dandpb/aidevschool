...
// QA Lead AID-1914 — walk de observação independente dojoToday (re-grant v53,
// PR #424, árvore 9aa3ec71). Superfícies: :5180 projeção canônica gerada
// (worktree — today.ts regenerada pelo PR com o catálogo 17 jogos),
// :5181/:5182 fixtures day-N/day-N+1 (seam DOJOTODAY_TODAY_MODULE, harness do
// próprio engine, AID-987/T1). Estações mapeiam as assertions "observation"
// dos 3 cenários do use case dojotoday-daily-guidance.
const SHOTS = "/tmp/opencode/aid1914/shots-dt";
const DAY_N = "http://127.0.0.1:5181";
const DAY_N_PLUS_1 = "http://127.0.0.1:5182";
const LOG: Record<string, unknown> = {
  base: ":5180 canônica (worktree 9aa3ec71) + fixtures :5181/:5182 (seam DOJOTODAY_TODAY_MODULE)",
  gitPin: "9aa3ec71",
};
...
test("obs dojotoday-active-unit-guidance: agenda + unidade ativa legíveis; guia, não avalia", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await page.screenshot({ path: `${SHOTS}/aid1914-dtobs-01-active-unit.png`, fullPage: true });
});

test("obs dojotoday-read-only-boundary (parte observada): saída bornada p/ projeção stale", async ({
  page,
}) => {
  ...
  await page.screenshot({ path: `${SHOTS}/aid1914-dtobs-02-readonly-boundary.png`, fullPage: true });
});

test("obs dojotoday-returning-next-day: day N → N+1 sem estado stale e sem avaliação", async ({
  page,
}) => {
  ...
  await page.screenshot({ path: `${SHOTS}/aid1914-dtobs-03-day-n.png`, fullPage: true });
  ...
  await page.screenshot({ path: `${SHOTS}/aid1914-dtobs-04-day-n-plus-1.png`, fullPage: true });
  writeFileSync("/tmp/opencode/aid1914/walk-log-dt-aid1914.json", JSON.stringify(LOG, null, 2));
});
