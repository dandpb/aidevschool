// FPE AID-1295 — walk de observação independente LITERACY LOCAL
// (vite @ main 25c0232f, re-anchor v43 pós-O3-C2). Estações mapeiam as
// assertions "observation" dos cenários readiness literacy-standalone-first-lesson
// e literacy-standalone-corridor-mod01-03 (padrão v42/AID-1265).
...
const SHOTS = "/tmp/opencode/aid1295/shots-lit";
const LOG: Record<string, unknown> = { base: "http://localhost:4190 (vite @ main 25c0232f)", walk: "v43 re-anchor pós-O3-C2 (AID-1295)" };
...
test("obs 1-3: próxima-lição nomeável, fronteira de dispositivo, retry sem jargão", async ({ page }) => {
  ...
  await page.screenshot({ path: `${SHOTS}/qa1295-litobs-01-welcome-footer.png`, fullPage: true });
  ...
  await page.screenshot({ path: `${SHOTS}/qa1295-litobs-02-wrong-hint.png`, fullPage: false });
  ...
  await page.screenshot({ path: `${SHOTS}/qa1295-litobs-03-result-next.png`, fullPage: true });
});
...
test("obs 4-6: pós-cp-03 próxima ação, janela de revisão distinta, desafio com retry", async ({ page }) => {
  ...
  await page.screenshot({ path: `${SHOTS}/qa1295-litobs-04-postcp03-home.png`, fullPage: true });
  await page.getByTestId("open-map").click();
  await expect(page.getByTestId("map-screen")).toBeVisible();
  LOG.postCp03MapNextModule = (await page.getByTestId("map-screen").innerText()).slice(0, 900);
  await page.screenshot({ path: `${SHOTS}/qa1295-litobs-05-postcp03-map.png`, fullPage: true });
  ...
  await page.screenshot({ path: `${SHOTS}/qa1295-litobs-06-review-distinct.png`, fullPage: false });
  ...
  if (await cp.count()) {
    ...
    await page.screenshot({ path: `${SHOTS}/qa1295-litobs-07-checkpoint.png`, fullPage: false });
  } else {
...
test.afterAll(async () => {
  console.log("QA1295_LIT_LOG=" + JSON.stringify(LOG, null, 1));
});
