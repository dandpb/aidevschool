// FPE AID-1803 — walk de observação independente OS (re-grant v51, PR #406 (re-anchor pós #407/main c2fe07a9))
// Superfície: http://127.0.0.1:4180 (vite preview do bundle piloto @ branch
// ux-dock-a11y-10924827111773933314, 735b98cd). Estações mapeiam as assertions
...
const SHOTS = "/tmp/opencode/aid1803/shots-os406c";
...
test("obs os-onboarding-track-choice + dock a11y (#406) + retorno + recuperação", async ({ page }) => {
  ...
  await page.screenshot({ path: `${SHOTS}/qa1803-osobs-01-onboarding-choice.png`, fullPage: false });
  ...
  await page.screenshot({ path: `${SHOTS}/qa1803-osobs-02-dock-a11y-running.png`, fullPage: false });
  ...
  await page.screenshot({ path: `${SHOTS}/qa1803-osobs-03-returning-hub.png`, fullPage: false });
  ...
  await page.screenshot({ path: `${SHOTS}/qa1803-osobs-04-cleared-onboarding.png`, fullPage: false });
});
...
test("obs os-literacy-hosted-mission: missão completa → resultado + próxima ação nomeáveis", async ({ page }) => {
  ...
  LOG.l30Mounted = true;
  await page.screenshot({ path: `${SHOTS}/qa1803-osobs-05-l30-mounted.png`, fullPage: false });
  ...
  await page.screenshot({ path: `${SHOTS}/qa1803-osobs-06-l30-result.png`, fullPage: true });
  ...
  await page.screenshot({ path: `${SHOTS}/qa1803-osobs-07-host-boundary.png`, fullPage: true });
});
...
test("obs os-verification-recovery: erro → dica + retry na própria UI, sem intervenção externa", async ({ page }) => {
  ...
  await page.screenshot({ path: `${SHOTS}/qa1803-osobs-08-l30-recovery.png`, fullPage: false });
  await retry.click();
  ...
  await page.screenshot({ path: `${SHOTS}/qa1803-osobs-09-literacy-returning.png`, fullPage: false });
});
...
test("obs os-voxel: WAREHOUSE live + resultado honesto + continuidade no mesmo dispositivo", async ({ page }) => {
  ...
  await page.screenshot({ path: `${SHOTS}/qa1803-osobs-10-warehouse-mounted.png`, fullPage: false });
  await page.reload();
  await expect(frame.getByTestId("hud-status")).toBeVisible({ timeout: 30_000 });
  ...
  await page.screenshot({ path: `${SHOTS}/qa1803-osobs-11-warehouse-result.png`, fullPage: true });
...
test("obs os-renderer: projeção acessível visível + fronteira canônica", async ({ page }) => {
  ...
  await page.screenshot({ path: `${SHOTS}/qa1803-osobs-12-accessible-affordance.png`, fullPage: false });
  ...
  writeFileSync("/tmp/opencode/aid1803/walk-log-os406c.json", JSON.stringify(LOG, null, 2));
});
