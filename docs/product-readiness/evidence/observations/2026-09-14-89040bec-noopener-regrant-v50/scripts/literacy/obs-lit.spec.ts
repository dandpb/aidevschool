// FPE AID-1797 — walk de observação independente LITERACY (re-grant v50, PR #407 (re-anchor pós #405/main))
...
const SHOTS = "/tmp/opencode/aid1797/shots-lit407b";
...
test("obs 1-3: fronteira de dispositivo, retry sem jargão, próxima-lição nomeável (+ #407 noopener)", async ({
  ...
  await page.screenshot({ path: `${SHOTS}/qa1797b-litobs-01-welcome-footer-noopener.png`, fullPage: true });
  ...
  await page.screenshot({ path: `${SHOTS}/qa1797b-litobs-02-wrong-hint-retry.png`, fullPage: false });
  ...
  await page.screenshot({ path: `${SHOTS}/qa1797b-litobs-03-result-next.png`, fullPage: true });
  writeFileSync("/tmp/opencode/aid1797/walk-log-lit407b.json", JSON.stringify(LOG, null, 2));
});
...
test("obs 4-6: desafio com retry sem linguagem interna, pós-cp-03 próxima ação, revisão distinta", async ({
  ...
  await page.screenshot({ path: `${SHOTS}/qa1797b-litobs-04-gate-retry.png`, fullPage: false });
  await cpRetry.click();
  ...
  await page.screenshot({ path: `${SHOTS}/qa1797b-litobs-05-postcp03-home.png`, fullPage: true });
  await page.getByTestId("open-map").click();
  await expect(page.getByTestId("map-screen")).toBeVisible();
  LOG.postCp03MapNextModule = (await page.getByTestId("map-screen").innerText()).slice(0, 900);
  await page.screenshot({ path: `${SHOTS}/qa1797b-litobs-06-postcp03-map.png`, fullPage: true });
  ...
  await page.screenshot({ path: `${SHOTS}/qa1797b-litobs-07-review-distinct.png`, fullPage: false });
  writeFileSync("/tmp/opencode/aid1797/walk-log-lit407b.json", JSON.stringify(LOG, null, 2));
});
