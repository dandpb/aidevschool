# Lesson: Concurrent Task Queue

> **Cycle:** 04_concurrent_task_queue · **Generated:** 2026-07-02 · **Levels:** intuition → formal → practical
> DoD §7: *"lesson.md explains the concept at 3 levels (intuition, formal, practical)."*

## Overview

Build an in-memory concurrent task queue service that accepts tasks from producers, stores them in bounded queues, dispatches them to a configurable worker pool, retries transient failures with backoff, and moves exhausted or poison tasks into a dead-letter queue.



## 1. Intuition

Concurrent Task Queue is a deceptively simple system whose difficulty lives in the edges: deterministic
