...
QuestData.glossary.push(
 ['Harness Lab e harness-toolkit','O laboratório de execução do Quest usa um motor local próprio para ensinar gates. O repositório dandpb/harness-toolkit foi indicado como referência, mas seu conteúdo não foi acessível nesta revisão; integração real pendente.','Harness Lab and harness-toolkit','The Quest execution lab uses its own local engine to teach gates. The dandpb/harness-toolkit repository was given as a reference, but its content was not accessible in this review; real integration pending.'],
 ['Recibo de execução','Registro de passo, entradas, revisão, política e checks executados. Um recibo local não é uma assinatura nem prova, sozinho, a identidade de quem executou.','Execution receipt','Record of step, inputs, review, policy and executed checks. A local receipt is not a signature and, alone, proves nothing about who ran it.'],
 ['Evidência desatualizada','Quando código, contrato ou política relevante muda, resultados dependentes precisam ser reavaliados. Um teste verde de outra revisão não libera a candidata atual.','Stale evidence','When relevant code, contracts or policy change, dependent results must be re-evaluated. A green test from another revision does not clear the current candidate.'],
 ['Ausência de resultado','Sem verificador, com erro ou sem checks, o gate não deve converter ausência de evidência em sucesso. Falha é registrada; um orçamento impede repetições automáticas ilimitadas.','Absence of result','With no verifier, an error or no checks, the gate must not turn missing evidence into success. The failure is recorded; a budget prevents unlimited automatic retries.']
);
