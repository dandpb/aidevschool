...
def prepare_workflow(
    ...
    return "\n".join(
        (
            ...
            f"Phase: {status.phase.value} · awaiting: {status.awaiting} · grade: {status.grade.value} · advanced_by: {status.advanced_by or '-'}",
            "Procedência: simulate e unspecified não substituem verified quando verificação independente é exigida.",
