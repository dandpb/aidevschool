...
| Parte | Estado |
| --- | --- |
| Conteúdo | O percurso público projeta 20 missões de `ia_pratica` em 6 módulos (`mod-01`–`mod-04`, `mod-06`–`mod-07`; contagem canônica em `curriculum/ai-literacy/catalog.yaml`). A Trilha Dev no onboarding é um CTA para o OS público (`?track=dev`); as 9 lições Dev do catálogo não entram neste app. |
