...
| `engines/codexdojo-os-prototype/bridge/` | Vite-development-only loopback bridge. It maps three exact engine action IDs plus the declared literacy evidence schema to fixed processes; callers cannot supply paths, commands, arguments, or verifier selection. Independent verification requires the integrated bridge flag. |
...
| `engines/codexdojo-os-prototype/config/mission-bindings.yaml` · `learner/substrate/mission_catalog.py` · `engines/codexdojo-os-prototype/src/data/missions.ts` | Host-owned runtime bindings joined to canonical ready lessons and project-linked Voxel games as a generated, read-only mission catalog. Bindings add launch/fallback metadata without copying content or mastery authority. |
| `engines/codexdojo-os-prototype/src/journey/` · `engines/codexdojo-os-prototype/src/progress/` · `engines/codexdojo-os-prototype/src/desktop/DesktopApp.tsx` | Mission-first onboarding and hub with one versioned local completion aggregate. The prior desktop remains available at `/desktop`; neither surface can write canonical mastery. |
| `engines/codexdojo-os-prototype/src/host/` · `engines/literacyDojo/src/host/` · `engines/shared/teaching-evidence/hostProtocol.ts` | Source-bound `aidevschool.host-engine` v1.0 handshake, correlated Literacy and Warehouse launches, revisioned local state, unchanged producer-evidence forwarding, acknowledgements, stale-run rejection, and cleanup. Transport acceptance remains separate from independent verification and canonical gate eligibility. |
| `engines/codexdojo-os-prototype/src/verification/` · `engines/codexdojo-os-prototype/bridge/verification.ts` · `learner/gate/literacy_bridge.py` · `learner/gate/literacy_verifier.py` | Schema-aware raw-evidence intake and opaque offline storage identity. Literacy evidence uses fixed independent verifier dispatch, a Python-owned canonical digest, identity-bound receipts, atomic receipt persistence, and retryable gateway failure. Teaching-game evidence is durably preserved as pending without a receipt until a schema-specific independent verifier exists. Producer PASS, local completion, verifier PASS, and canonical mastery remain distinct; this flow never invokes the canonical gate. |
...
| **OS mission catalog and host** | `engines/codexdojo-os-prototype/config/mission-bindings.yaml` · `learner/substrate/mission_catalog.py` · `engines/codexdojo-os-prototype/src/data/missions.ts` · `engines/codexdojo-os-prototype/src/host/` · `engines/literacyDojo/src/host/` | Joins stable lesson identity to launch metadata and runs the first IA Pratica mission through a correlated, origin-bound protocol. Local completion, raw evidence, independent receipts, and the generated learner snapshot remain structurally separate. |
| **OS literacy verification** | `engines/codexdojo-os-prototype/src/verification/` · `engines/codexdojo-os-prototype/bridge/verification.ts` · `learner/gate/literacy_bridge.py` · `learner/gate/literacy_verifier.py` | Sends unchanged `LiteracyEvidenceRecord` values to one fixed verifier, lets Python own the stable digest, and atomically binds persisted raw evidence to independent PASS/FAIL receipts. Browser input cannot select a process or canonical path; canonical gating remains a separate substrate-owned operation. |
...
| **Voxel internal kit** | `engines/voxelDojo/shared/sceneHarness.ts` · `engines/voxelDojo/shared/viewport.ts` | Shared Three.js lifecycle and viewport behavior within the Voxel engine-local workspace. |
