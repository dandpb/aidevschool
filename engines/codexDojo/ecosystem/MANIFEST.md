...
| **Engine adapter registry** | `engines/codexdojo-os-prototype/src/engines/registry.ts` · `engines/codexdojo-os-prototype/bridge/` · `engines/shared/teaching-evidence/emit.ts` | Keeps browser engines isolated, pins local actions to a loopback-only allowlist, and transports raw teaching evidence to the host without granting verifier authority. |
