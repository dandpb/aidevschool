...
| **OS mission catalog and host** | `engines/codexdojo-os-prototype/config/mission-bindings.yaml` · `learner/substrate/mission_catalog.py` · `engines/codexdojo-os-prototype/src/data/missions.ts` · `engines/codexdojo-os-prototype/src/journey/MapScreen.tsx` · `engines/codexdojo-os-prototype/src/journey/studentPath.ts` · `engines/codexdojo-os-prototype/src/host/indexedDbCheckpointStore.ts` · `engines/literacyDojo/src/host/` · `engines/shared/teaching-evidence/hostProtocol.ts` | Runs the twenty-mission IA Prática track (`l01`–`l14` plus the module-06 continuation lessons `l18`–`l20` and the module-07 O2-wave lessons `l24`–`l26`), the hosted Dev track (literacy missions `l15`–`l17`, `l21`–`l23`, and `l27`–`l29` plus the WAREHOUSE, WORMHOLE, RELAY STATION, and PIPELINE PLANT simulations) as one single-sequence chapter map through a correlated, origin-bound contract. Legacy dev-track selections are remapped losslessly; opaque checkpoints are version-compatible only; local completion, raw evidence, independent receipts, and generated canonical state remain structurally separate. |
