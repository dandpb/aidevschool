...
| `engines/codexdojo-os-prototype/config/mission-bindings.yaml` · `learner/substrate/mission_catalog.py` · `engines/codexdojo-os-prototype/src/data/missions.ts` | Host-owned runtime bindings join the twenty IA Prática lessons (`l01`–`l14`, modules 01–04, plus `l18`–`l20` in module 06 per the C1–C3 continuation wave and `l24`–`l26` in module 07 per the O2 wave), the six Dev-journey lessons (`l15`–`l17` and `l21`–`l23`, module 05, bound to the `dev` track per canonical module `journey` — the l21–l23 wave covers tests, code review, and acceptance limits with AI), and four project-linked Voxel games into the generated, read-only catalog. Each track declares a contiguous `chapterOrder` run from 1; chapter order, recommended entry, prerequisites, stable versions, and semantic fallback metadata are validated without copying lesson/game content or mastery authority. |
