...
| `engines/voxelDojo/` | 3D teaching-simulation engine (Three.js): structures/dynamics concepts become operable 3D systems emitting evidence with `source: "voxeldojo"`. Pilot `game-10-hash-ring/` (consistent hashing, unit `U9-distributed-cache`) is `implemented`: headless sim core with Vitest concept proofs, Playwright smoke at `game-10-hash-ring/playwright/hash-ring.spec.ts`, substrate review slice at `game-10-hash-ring/src/content/reviewSlice.ts`. Cross-engine rules: `docs/design/teaching-game-contract.md`. |
