...
| `engines/codexdojo-os-prototype/config/mission-bindings.yaml` · `learner/substrate/mission_catalog.py` · `engines/codexdojo-os-prototype/src/data/missions.ts` | Host-owned runtime bindings join the twenty-three IA Prática lessons (`l01`–`l14`, modules 01–04, plus `l18`–`l20` in module 06 per the C1–C3 continuation wave, `l24`–`l26` in module 07 per the O2 wave, and `l30`–`l32` in module 08 per the W1 "Rotina com IA II" wave — assisted routines with human control: what to automate, where the human validates, and when automation degrades), the nine Dev-journey lessons (`l15`–`l17` and `l21`–`l23` plus `l27`–`l29`, module 05, bound to the `dev` track per canonical module `journey` — the l21–l23 wave covers tests, code review, and acceptance limits with AI; the O1 assisted-maintenance wave opens with `l27` reproducible bug reports, continues with `l28` refactoring that preserves behavior, and closes with `l29` judging suggested dependencies), and four project-linked Voxel games into the generated, read-only catalog. Each track declares a contiguous `chapterOrder` run from 1; chapter order, recommended entry, prerequisites, stable versions, and semantic fallback metadata are validated without copying lesson/game content or mastery authority. |
...
| `engines/codexdojo-os-prototype/src/host/` · `engines/literacyDojo/src/host/` · `engines/shared/teaching-evidence/hostProtocol.ts` | Source-bound `aidevschool.host-engine` v1.0 handshake for every published chapter mission, revisioned state, unchanged producer-evidence forwarding, acknowledgements, stale-run rejection, and complete replacement cleanup. Opaque progress state persists in a per-engine IndexedDB database via `engines/codexdojo-os-prototype/src/progress/indexedDbProgressRepository.ts` and is re-admitted on load only when mission-version compatible per `engines/codexdojo-os-prototype/src/progress/migration.ts`; transport acceptance remains separate from verification and canonical gate eligibility. |
| `engines/codexdojo-os-prototype/src/verification/` · `engines/codexdojo-os-prototype/bridge/verification.ts` · `learner/gate/literacy_bridge.py` · `learner/gate/teaching_game_bridge.py` | Schema-aware raw-evidence intake and opaque offline storage identity. Literacy records and the four fixed WAREHOUSE, WORMHOLE, RELAY STATION, and PIPELINE PLANT identities use fixed independent verifier dispatch, Python-owned canonical digests, identity-bound receipts, atomic receipt persistence, and retryable gateway failure. Each teaching-game evaluator recomputes metrics and pass from a closed raw-observation trace, then explicitly reports that canonical gate submission still requires a separate learner attempt and normal eligibility checks. Producer PASS, local completion, verifier PASS, and canonical mastery remain distinct; this flow never writes canonical mastery. |
...
| `engines/codexdojo-os-prototype/src/rendering/` · `engines/voxelDojo/shared/projection.ts` · `engines/voxelDojo/shared/accessibleProjection.ts` · `engines/voxelDojo/shared/viewport.ts` | Evidence-neutral renderer lifecycle for the four hosted Dev missions. The correlated launch declares reduced-motion and `auto | webgl | accessible` preference; revisioned renderer state reports probing, initialization, readiness, degradation, and failure separately from mission state. Guarded WebGL construction, bounded initialization, context-loss disposal, retry, and labelled DOM projections preserve the same controller, deterministic snapshot, completion rules, and raw evidence identity. Pixel ratio remains capped at 2, and rendering never evaluates or emits evidence. |
...
| `engines/voxelDojo/` | 3D teaching-simulation engine (Three.js): structures/dynamics concepts become operable 3D systems emitting evidence with `source: "voxeldojo"`. **17 spatial-concept games implemented** (2026-07-05 all-18 buildout; `game-04-task-queue` TASK FORGE added 2026-09-14 via AID-1901): `game-02-warehouse`, `game-03-wormhole`, `game-04-task-queue`, `game-05-relay-station`, `game-06-pipeline-plant`, `game-07-checkpoint-city`, `game-08-timeline-tower`, `game-09-docking-bay`, `game-10-hash-ring` (pilot), `game-11-air-traffic`, `game-12-mission-control`, `game-13-breaker-grid`, `game-14-river-delta`, `game-15-observatory`, `game-16-freight-yard`, `game-17-lighthouse-network`, `game-18-stacks`. Each ships a deterministic headless sim core + Vitest concept proofs + Three.js scene + 4 levels + voxeldojo evidence emit + Playwright smoke; per-game PLAN slice in `docs/plans/` (`game-04-task-queue` records its slice in `DECISIONS.md` instead — its curriculum plan slice remains the pixel-side `engines/pixelDojo/docs/plans/04_concurrent_task_queue.md`). The rate-limiter concept (01) lives in pixel-quest as a Shape A encounter; the task-queue concept (04) ships both as the pixel-quest Shape A encounter and as voxelDojo `game-04-task-queue`. Cross-engine rules: `docs/design/teaching-game-contract.md`. |
...
| --- | --- | --- |
| **Evidence classes and context authority** | `CONTEXT-MAP.md` · `learner/CONTEXT.md` · `learner/gate/no_code.py` · `learner/substrate/gate.py` · `learner/substrate/tests/test_gate_evidence_class.py` | Separates browser completion/receipts from canonical no-code promotion. No-code success labels independently verified no-code evidence and omits executable-code thresholds; code gates retain their contract. Verified learning streak and local engagement streak remain distinct. |
| **Canonical commit and publication** | `learner/substrate/interface.md` · `learner/substrate/__init__.py` · `learner/substrate/projections.py` | Documents synchronous build-before-write, per-file atomic publication, possible stale views after I/O failure, and recovery through drift checks and regeneration without replaying a consumed gate. |
| **Pipeline provenance and MVP authority** | `engines/openclaw/runner/pipeline_status.py` · `engines/openclaw/tests/test_provenance.py` · `engines/miniMaxEvolutionEngine/tests/test_context_authority.py` · `engines/miniMaxEvolutionEngine/tests/test_authority_contract_semantics.py` · `CONTEXT-MAP.md` · `learner/CONTEXT.md` · `engines/miniMaxEvolutionEngine/CONTEXT.md` · `engines/aiDevschoolMvp/README.md` | Closed grade/writer provenance distinguishes checklist advances from independently verified phases; legacy remains unspecified. MVP mastery, G1–G4 gates and gap-ladder reviews remain separate from canonical mastery and FSRS. |
...
| **OS mission catalog and host** | `engines/codexdojo-os-prototype/config/mission-bindings.yaml` · `learner/substrate/mission_catalog.py` · `engines/codexdojo-os-prototype/src/data/missions.ts` · `engines/codexdojo-os-prototype/src/journey/MapScreen.tsx` · `engines/codexdojo-os-prototype/src/journey/studentPath.ts` · `engines/codexdojo-os-prototype/src/progress/indexedDbProgressRepository.ts` · `engines/codexdojo-os-prototype/src/progress/migration.ts` · `engines/literacyDojo/src/host/` · `engines/shared/teaching-evidence/hostProtocol.ts` | Runs the twenty-three-mission IA Prática track (`l01`–`l14` plus the module-06 continuation lessons `l18`–`l20`, the module-07 O2-wave lessons `l24`–`l26`, and the module-08 W1-wave lessons `l30`–`l32`), the hosted Dev track (literacy missions `l15`–`l17`, `l21`–`l23`, and `l27`–`l29` plus the WAREHOUSE, WORMHOLE, RELAY STATION, and PIPELINE PLANT simulations) as one single-sequence chapter map through a correlated, origin-bound contract. Legacy dev-track selections are remapped losslessly; opaque checkpoints are version-compatible only; local completion, raw evidence, independent receipts, and generated canonical state remain structurally separate. |
...
| **Literacy funnel analytics (AID-913 activation)** | `engines/literacyDojo/src/domain/analytics.ts` · `engines/literacyDojo/src/adapters/analyticsBatchSink.ts` · `engines/literacyDojo/src/adapters/analyticsIdentity.ts` · `learner/gate/netlify-functions/dojo-analytics-collector.mjs` · `learner/gate/analytics/aggregate_funnel.mjs` | Measures the standalone first-lesson funnel (entry → lesson started → activity attempted → lesson completed) with an ephemeral anonymous per-page-load session id and per-event event id under a closed per-event prop vocabulary (schemaVersion 2, source literacydojo). Buffered same-origin batches degrade silently and never block the lesson; the same-origin collector persists idempotently (Netlify Blobs when present) and exports token-guarded raw NDJSON (90-day retention); aggregation publishes only k≥5-suppressed, identifier-free funnel cells. Analytics never writes learner state, evidence, gates, or mastery.|
| **Programmer-surface funnel analytics (AID-987/T1b)** | `engines/shared/teaching-evidence/funnelTelemetry.ts` · `engines/shared/teaching-evidence/evidenceTransport.ts` · `engines/dojoToday/src/main.ts` · `learner/gate/netlify-functions/dojo-analytics-collector.mjs` · `learner/gate/analytics/schema_drift_monitor.mjs` | Measures the dojoToday/voxelDojo/PixelQuest funnels (daily-view-open; voxel-loop-complete; pixelquest-encounter-complete; evidence-handoff on real host handoff) with an anonymous, in-memory per-page-load session id under closed per-event prop vocabularies (schemaVersion 3 surfaces envelope through the SAME collector). Emission activates only where a deploy sets the same-origin `VITE_ANALYTICS_ENDPOINT`; local/dev builds stay silent no-ops, failures are dropped silently, and no learner identity, free text, or persisted identifier ever leaves the surface. Analytics never writes learner state, evidence, gates, or mastery.|
| **Engine adapter registry** | `engines/codexdojo-os-prototype/src/engines/registry.ts` · `engines/voxelDojo/catalog.json` · `engines/codexdojo-os-prototype/src/engines/voxelCatalog.ts` · `engines/codexdojo-os-prototype/bridge/` · `engines/minimaxDojo/os_adapter.py` · `engines/miniMaxEvolutionEngine/os_adapter.py` · `engines/shared/teaching-evidence/emit.ts` | Integrates six external engines: isolated browser origins, all 17 voxel games from the voxel-owned catalog, authenticated loopback-only local workflow briefings, and raw teaching evidence transport without verifier authority. |
