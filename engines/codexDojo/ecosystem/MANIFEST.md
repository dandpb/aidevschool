...
| `engines/codexdojo-os-prototype/config/mission-bindings.yaml` · `learner/substrate/mission_catalog.py` · `engines/codexdojo-os-prototype/src/data/missions.ts` | Host-owned runtime bindings join the fourteen IA Prática lessons (`l01`–`l14`, modules 01–04) and four project-linked Voxel games into the generated, read-only catalog. Each track declares a contiguous `chapterOrder` run from 1; chapter order, recommended entry, prerequisites, stable versions, and semantic fallback metadata are validated without copying lesson/game content or mastery authority. |
