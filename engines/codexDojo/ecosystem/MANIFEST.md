...
| `engines/codexdojo-os-prototype/src/host/` · `engines/literacyDojo/src/host/` · `engines/shared/teaching-evidence/hostProtocol.ts` | Source-bound `aidevschool.host-engine` v1.0 handshake for every published chapter mission, revisioned state, unchanged producer-evidence forwarding, acknowledgements, stale-run rejection, and complete replacement cleanup. Opaque checkpoints are isolated by engine, mission version, and runtime content version in `src/host/indexedDbCheckpointStore.ts`; transport acceptance remains separate from verification and canonical gate eligibility. |
...
| **OS mission catalog and host** | `engines/codexdojo-os-prototype/config/mission-bindings.yaml` · `learner/substrate/mission_catalog.py` · `engines/codexdojo-os-prototype/src/data/missions.ts` · `engines/codexdojo-os-prototype/src/journey/MapScreen.tsx` · `engines/codexdojo-os-prototype/src/journey/studentPath.ts` · `engines/codexdojo-os-prototype/src/host/indexedDbCheckpointStore.ts` · `engines/literacyDojo/src/host/` · `engines/shared/teaching-evidence/hostProtocol.ts` | Runs the twenty-three-mission IA Prática track (`l01`–`l14` plus the module-06 continuation lessons `l18`–`l20`, the module-07 O2-wave lessons `l24`–`l26`, and the module-08 W1-wave lessons `l30`–`l32`), the hosted Dev track (literacy missions `l15`–`l17`, `l21`–`l23`, and `l27`–`l29` plus the WAREHOUSE, WORMHOLE, RELAY STATION, and PIPELINE PLANT simulations) as one single-sequence chapter map through a correlated, origin-bound contract. Legacy dev-track selections are remapped losslessly; opaque checkpoints are version-compatible only; local completion, raw evidence, independent receipts, and generated canonical state remain structurally separate. |
