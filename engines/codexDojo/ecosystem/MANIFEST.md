...
| `engines/codexdojo-os-prototype/config/mission-bindings.yaml` · `learner/substrate/mission_catalog.py` · `engines/codexdojo-os-prototype/src/data/missions.ts` | Host-owned runtime bindings join the sixteen IA Prática lessons (`l01`–`l14`, modules 01–04, plus `l18`–`l19` in module 06 per the C1–C3 continuation wave), the three Dev-journey lessons (`l15`–`l17`, module 05, bound to the `dev` track per canonical module `journey`), and four project-linked Voxel games into the generated, read-only catalog. Each track declares a contiguous `chapterOrder` run from 1; chapter order, recommended entry, prerequisites, stable versions, and semantic fallback metadata are validated without copying lesson/game content or mastery authority. |
...
| **OS mission catalog and host** | `engines/codexdojo-os-prototype/config/mission-bindings.yaml` · `learner/substrate/mission_catalog.py` · `engines/codexdojo-os-prototype/src/data/missions.ts` · `engines/codexdojo-os-prototype/src/journey/MapScreen.tsx` · `engines/codexdojo-os-prototype/src/journey/studentPath.ts` · `engines/codexdojo-os-prototype/src/host/indexedDbCheckpointStore.ts` · `engines/literacyDojo/src/host/` · `engines/shared/teaching-evidence/hostProtocol.ts` | Runs the sixteen-mission IA Prática track (`l01`–`l14` plus the module-06 continuation lessons `l18`–`l19`), the hosted Dev track (literacy missions `l15`–`l17` plus the WAREHOUSE, WORMHOLE, RELAY STATION, and PIPELINE PLANT simulations) as one single-sequence chapter map through a correlated, origin-bound contract. Legacy dev-track selections are remapped losslessly; opaque checkpoints are version-compatible only; local completion, raw evidence, independent receipts, and generated canonical state remain structurally separate. |
