...
| `engines/pixelDojo/` | 8-bit learning-game engine. `pixel-quest/` is the Three.js RPG attempt surface; `codexdojo-ecosystem-manifest/` is the raw Canvas manifesto game. |
| `docs/` | Existing polyglot MiniMax/OpenClaw/Hermes documentation. |
| `learner/` | Canonical learner-state substrate (`learner/substrate/`); single source of truth for all engines. |
| `.mavis/` | Derived learning-state view generated by `learner/substrate/adapters/mavis.py`. |
| `.claude/` | Claude Code agent adapters and commands for the same protocol. |
| `engines/miniMaxEvolutionEngine/.claude/commands/devschool/phaserunner.md` | Reusable `run_phase(spec)` seam; every phase command is a thin spec invocation. |
...
| 5 | Learning memory model | `engines/codexDojo/ecosystem/MEMORY_MODEL.md`, `engines/minimaxDojo/docs/05_memory_system.md`, `learner/`, `learner/substrate/` (canonical state + adapters for `.mavis/` and `whiteboard/`) |
...
| 12 | Ludic learning surface with extensible content | `engines/pixelDojo/pixel-quest/README.md`, `engines/pixelDojo/pixel-quest/docs/content-packs.md`, `engines/pixelDojo/pixel-quest/src/content/packs/core/pack.json`, `engines/pixelDojo/codexdojo-ecosystem-manifest/README.md`, `engines/pixelDojo/codexdojo-ecosystem-manifest/docs/manifest.md` |
...
| Ludic practice and evidence capture | `engines/pixelDojo/pixel-quest/`, `engines/pixelDojo/pixel-quest/playwright/pixel-quest.spec.ts`, `engines/pixelDojo/codexdojo-ecosystem-manifest/` |
...
| --- | --- | --- |
| **PhaseRunner** | `engines/miniMaxEvolutionEngine/.claude/commands/devschool/phaserunner.md` | The repeated read-state → check-gate → dispatch producer → dispatch verifier → update status → retry pattern. Every `/devschool-*` command is a thin YAML `run_phase(spec)` invocation. |
| **Learner substrate** | `learner/substrate/` (`__init__.py`, `schema.yaml`, `adapters/{mavis,whiteboard}.py`, `tests/`) | Canonical learner state in `learner/learning_state.yaml` with derived views for `.mavis/`, `engines/minimaxDojo/whiteboard/`, and the Markdown profile. `python3 -m learner.substrate` regenerates; `python3 -m unittest discover -s learner/substrate/tests -t .` validates invariants. |
