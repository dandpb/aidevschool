...
| Technology comparison coverage | Complete | `CURRICULUM_SCOPE.md`, `EVALUATION_MODELS.md`, `curriculum/catalog.md` |
| Robust application construction | Complete | `ROADMAP.md`, `curriculum/AGENTS.md`, `curriculum/01_rate_limiter/` |
| Architecture models | Complete | `CURRICULUM_SCOPE.md`, `OPERATING_MODEL.md`, `engines/minimaxDojo/docs/00_architecture.md` |
| Code review and quality model | Complete | `EVALUATION_MODELS.md`, `curriculum/01_rate_limiter/docs/code_review.md`, `engines/minimaxDojo/docs/04_empirical_gates.md` |
...
| Folder structure for project packages | Complete | `ROADMAP.md` |
| Learning memory model | Complete | `MEMORY_MODEL.md`, `learner/`, `engines/minimaxDojo/docs/05_memory_system.md` |
| Local dashboard app | Complete | `engines/codexDojo/src/`; execute `pnpm run lint`, `pnpm run test`, and `pnpm run build` from the engine root. Current counts are recorded by the dated refresh below. |
...
Historical results (2026-06-21; retained for audit trail):
...
Current mechanically reproducible refresh (2026-07-26):

- `pnpm run test` passed **18 test files** with **88 tests**.
- `python3 -m unittest discover -s learner/substrate/tests -t .` passed **149 tests**.
- `python3 -m pytest learner/substrate/tests/test_engine_topology.py learner/substrate/tests/test_mission_catalog_graph_rules.py -q` passed **4 tests** and **14 subtests**.
- Engine roots: **11 directories**, from `find engines -mindepth 1 -maxdepth 1 -type d ! -name graphify-out ! -name shared ! -name __pycache__ -print | sort`.
...
Historical observed counts (2026-06-21; retained for audit trail):
...
| Substrate tests (historical snapshot 2026-06-21) | 34 | `learner/substrate/tests/` |
| Engine topology test methods (historical snapshot) | 2 | `learner/substrate/tests/test_engine_topology.py` |
| Phaserunner tests | 9 | `.claude/commands/devschool/tests/test_phaserunner.py` |
| codexDojo app tests (historical snapshot 2026-06-21) | 55 | `engines/codexDojo/src/*.test.ts` |
| Engine roots (historical snapshot 2026-06-21) | 4 | `codexDojo`, `minimaxDojo`, `miniMaxEvolutionEngine`, `pixelDojo` (was 6 before 2026-06-21 consolidation) |
