    ...
    <section class="workbench agents-view" aria-label="Agentes do codexDojo">
      ...
      <article class="agent-detail">
        <div class="detail-topline">
          ...
          <button class="icon-button" type="button" data-copy-agent="${escapeHtml(selectedAgent.id)}" aria-live="polite" aria-label="${state.copiedAgentId === selectedAgent.id ? "Prompt copiado" : `Copiar prompt de ${escapeHtml(selectedAgent.name)}`}" title="${state.copiedAgentId === selectedAgent.id ? "Copiado" : "Copiar prompt"}">
