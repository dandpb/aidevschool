        ...
        <div class="timeline">
          ${getStages()
            .map((stage, index) => {
              ...
              const ariaLabel = `Etapa ${index + 1}, ${stage.label}, status: ${statusText}`
              return `
                <button class="timeline-step ${className} ${completed}" type="button" data-stage="${escapeHtml(stage.id)}" aria-label="${escapeHtml(ariaLabel)}"${aria}>
