    ...
    <div class="learner-aidi">
      ...
      <div class="aidi-row">
        <div class="${signalClass}">
          <strong>${escapeHtml(typeof aidi.current === "number" ? aidi.current.toFixed(2) : aidi.current)}</strong>
          <small>alerta amarelo em ${escapeHtml(typeof aidi.thresholdAmber === "number" ? aidi.thresholdAmber.toFixed(2) : aidi.thresholdAmber)} · vermelho em ${escapeHtml(typeof aidi.thresholdRed === "number" ? aidi.thresholdRed.toFixed(2) : aidi.thresholdRed)}</small>
        </div>
...
function renderStreak(snapshot: LearnerSnapshot): string {
  ...
  const filled = "❄".repeat(Number(s.freezesEquipped) || 0)
  const empty = "·".repeat(
...
function renderCurr(snapshot: LearnerSnapshot): string {
  return `
    <div class="learner-curr">
      ...
      <strong class="curr-value">${escapeHtml(typeof snapshot.curr === "number" ? snapshot.curr.toFixed(2) : snapshot.curr)}</strong>
      <p class="curr-hint">
