...
function renderStreak(snapshot: LearnerSnapshot): string {
  ...
  const filled = "❄".repeat(typeof s.freezesEquipped === "number" ? s.freezesEquipped : 0)
  const empty = "·".repeat(
    Math.max(
      0,
      (typeof s.freezesMax === "number" ? s.freezesMax : 0) -
        (typeof s.freezesEquipped === "number" ? s.freezesEquipped : 0),
    ),
  )
