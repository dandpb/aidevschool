    ...
    <div class="learner-aidi">
      ...
      <div class="aidi-row">
        <div class="${signalClass}">
          <strong>${escapeHtml(aidi.current.toFixed(2))}</strong>
          <small>alerta amarelo em ${escapeHtml(aidi.thresholdAmber.toFixed(2))} · vermelho em ${escapeHtml(aidi.thresholdRed.toFixed(2))}</small>
        </div>
      ...
      <small class="aidi-source">Últimos ${escapeHtml(aidi.trend.length)} pontos — atualize rodando <code>python3 -m learner.substrate</code></small>
    </div>
  `
}
...
function renderCurr(snapshot: LearnerSnapshot): string {
  return `
    <div class="learner-curr">
      ...
      <strong class="curr-value">${escapeHtml(snapshot.curr.toFixed(2))}</strong>
      <p class="curr-hint">
...
export function renderLearnerDashboard(): string {
  ...
  const points = `${escapeHtml(snapshot.aidi.trend.length)}/${escapeHtml(AIDI_HISTORY_POINTS)}`
    ...
    <section class="learner-dashboard" aria-label="Painel do aprendiz">
      <div class="section-heading">
        ...
        <p>
          ...
          <code>python3 -m learner.substrate</code>. <span class="trend-points">${escapeHtml(points)} pontos de AIDI</span>.
        </p>
      </div>
