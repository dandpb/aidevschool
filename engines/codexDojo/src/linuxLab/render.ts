...
export function renderLinuxLab(_state: AppState): string {
  ...
  const action = osUrl
    ? `<a class="action-button" data-codexdojo-os-launch="true" href="${escapeHtml(osUrl)}" target="_blank" rel="noopener noreferrer" aria-label="Abrir codexDojo OS (abre em nova aba)"><span aria-hidden="true">Abrir codexDojo OS</span></a>`
