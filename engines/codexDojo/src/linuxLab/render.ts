...
export function renderLinuxLab(_state: AppState): string {
  ...
  const action = osUrl
    ? `<a class="action-button" data-codexdojo-os-launch="true" href="${escapeHtml(osUrl)}" target="_blank" rel="noopener noreferrer" title="Abrir codexDojo OS">Abrir codexDojo OS <span class="sr-only">(abre em nova aba)</span></a>`
