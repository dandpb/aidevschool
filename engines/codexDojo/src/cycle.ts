...
// ⚡ Bolt: Pre-compute Map for O(1) index lookups to avoid O(n) .findIndex scans on every interaction
...
export function advanceCycle(snapshot: CycleSnapshot): CycleSnapshot {
  const selectedIndex = cycleStageIndexById.get(snapshot.selectedStageId) ?? -1
