...
// ⚡ Bolt: Pre-compute stage indices for O(1) lookups instead of O(n) array scans during state evaluation
const cycleStageIndices = new Map<string, number>()
...
export function advanceCycle(snapshot: CycleSnapshot): CycleSnapshot {
  const selectedIndex = cycleStageIndices.get(snapshot.selectedStageId) ?? -1
