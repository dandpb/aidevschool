...
function evidenceLabel(state: EvidenceVerificationState | undefined): string {
  ...
  if (state.kind === 'verified')
    return state.receipt.verdict === 'PASS' ? 'Veredito PASS' : 'Veredito FAIL'
    ...
    <main className="journey-page progress-page">
      <header className="progress-header">
        ...
        <div>
          ...
          <p>
            Cada coluna mostra quem produz o estado. XP, prática e sequência ficam neste
            dispositivo; domínio vem apenas da projeção canônica.
          </p>
        </div>
      </header>
      ...
      <section className="progress-summary" aria-label="Resumo de engajamento local">
        <div>
          ...
          <small>Fonte: OS / IndexedDB</small>
        </div>
        <div>
          ...
          <small>Fonte: OS / data local</small>
        </div>
        <div>
          <span>Sequência local</span>
          <strong>{progress.localEngagementStreak.current} dias</strong>
          <small>Fonte: práticas locais</small>
        </div>
        <div>
          <span>Competências</span>
          <strong>{learner.masteredCount}</strong>
          <small>Fonte: learner/substrate</small>
        </div>
      </section>
      ...
      <section className="progress-explainer" aria-label="Como ler o progresso">
        <article>
          ...
          <p>
            Estado local salvo no dispositivo. Pode liberar a próxima missão, mas não certifica
            domínio.
          </p>
        </article>
        <article>
          <span>2</span>
          <h2>Prática realizada</h2>
          <p>
            Marca que uma tentativa aplicou o conceito. Continua sendo um registro de engajamento
            local.
          </p>
        </article>
        <article>
          <span>3</span>
          <h2>Evidência preservada</h2>
          <p>Registro bruto do produtor, guardado fora do XP e sem poder de aprovar a si mesmo.</p>
        </article>
        <article>
          <span>4</span>
          <h2>Veredito independente</h2>
          <p>
            Recibo separado e ligado ao digest da evidência. PASS ainda respeita a elegibilidade do
            gate.
          </p>
        </article>
        <article>
          <span>5</span>
          <h2>Competência canônica</h2>
          <p>Somente a projeção de `learner/learning_state.yaml` pode apresentar este estado.</p>
        </article>
      </section>
      ...
      <section className="progress-missions" aria-label="Estado por missão">
        ...
        {catalog.listLaunchable().map((mission) => {
          ...
          return (
            <article key={key}>
              <div>
                <small>{mission.trackId === 'dev' ? 'Trilha Dev' : 'IA Prática'}</small>
                <strong>{mission.title}</strong>
              </div>
              <div>
                <span>Conclusão local</span>
                <strong>{localStatus === 'completed' ? 'Concluída' : localStatus}</strong>
              </div>
              <div>
                <span>Prática</span>
                <strong>{engagement?.practiceCompleted ? 'Realizada' : 'Pendente'}</strong>
              </div>
              <div>
                <span>Verificação</span>
                <strong>{evidenceLabel(verification)}</strong>
              </div>
              <div>
                <span>Canônico</span>
                <strong>
                  {missionHasCanonicalMastery(mission, learner)
