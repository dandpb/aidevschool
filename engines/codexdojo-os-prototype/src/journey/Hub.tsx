...
export function Hub({
  ...
  const {
    ...
    setVerification,
    verificationByKey,
  } = useVerificationByMission(catalog, services.verification)
  ...
  const status =
    mission === undefined
      ? undefined
      : progress.missionStatusByKey[missionKey(mission.trackId, mission.id)]
  const activeTrackId =
  ...
  const verification =
    completedMission === undefined
      ...
      : (verificationByKey[missionKey(completedMission.trackId, completedMission.id)] ?? {
          kind: 'not-submitted',
        })
  const independentlyVerified = verification.kind === 'verified'
  const now = services.clock()
  const todayXp = dailyXp(progress, now)
  const completedInTrack = catalog
    .listLaunchable(activeTrackId)
    .filter(
  ...
  const launchOptions: MissionStartOptions =
  ...
  return (
    <main className="journey-page hub-page">
      <header className="hub-topbar">
        <div className="journey-brand">
          ...
          <span>
            ...
            <small>hub de aprendizagem</small>
          </span>
        </div>
        <div className="hub-chips">
          <span>
            Trilha <strong>{progress.activeTrackId === 'dev' ? 'Dev' : 'IA Prática'}</strong>
          </span>
          <span>
            XP local <strong>{progress.xp}</strong>
          </span>
          <span>
            Meta{' '}
            <strong>
      ...
      <section className="hub-welcome">
        ...
        <p>
          Uma missão em destaque, com prática observável e progresso que não confunde conclusão com
          competência.
        </p>
      </section>
      ...
      {completedMission !== undefined ? (
        <section className="honest-progress" aria-label="Estado da missão concluída">
          <div>
            ...
            <small>Salva neste dispositivo</small>
          </div>
          <div className={verification.kind === 'not-submitted' ? '' : 'current'}>
            <span>2</span>
            <strong>Evidência preservada</strong>
            <small>
              {verification.kind === 'not-submitted'
                ? 'Ainda não enviada'
                : 'Separada do progresso local'}
            </small>
          </div>
          <div className={independentlyVerified ? 'current' : ''}>
            <span>3</span>
            <strong>Verificação independente</strong>
            <small>
              {independentlyVerified
                ? `Veredito ${verification.receipt.verdict}`
          ...
          <div>
            <span>4</span>
            <strong>Competência canônica</strong>
            <small>Não alterada por este fluxo</small>
          </div>
          {verification.kind === 'gateway-unavailable' ? (
            <button
              type="button"
              onClick={() =>
                void services.verification.retry(completedMission, (state) => {
      ...
      <section className="hub-grid">
        <article className="next-mission-card">
          {mission === undefined ? (
            <>
              ...
              <p>
                Não há outra missão disponível para esta trilha neste momento. Seu progresso foi
                preservado.
              </p>
            </>
          ) : (
            <>
              ...
              <p className="mission-expected">
                <strong>Resultado esperado:</strong> {mission.fallback.summary}
              </p>
              <ol className="mission-stages">
                <li>
                  <span>1</span>Entender
                </li>
                <li>
                  <span>2</span>Responder
                </li>
                <li>
                  <span>3</span>Aplicar
                </li>
              ...
              <button
                ...
                onClick={() => onLaunch(mission, launchOptions)}
              >
                {actionLabel}
              </button>
            </>
          )}
          <div className="hub-voxels" aria-hidden="true">
            <i />
            <i />
            <i />
            <i />
        ...
        <aside className="hub-side">
          ...
          <article>
            ...
            <h2>
              {dailyGoalMet(progress, now)
                ? 'Meta de hoje alcançada'
                : `${Math.max(0, progress.dailyGoalXp - todayXp)} XP para a meta`}
            </h2>
            <p>{streakCopy}</p>
          ...
          <article>
            ...
            <p>
              O jogo e o mentor ajudam a produzir uma tentativa. A verificação independente é um
              requisito; este fluxo não altera o estado canônico.
            </p>
            <strong>{learner.masteredCount} competências verificadas</strong>
            <button type="button" className="hub-map-link" onClick={onOpenProgress}>
              Entender meu progresso
            </button>
          </article>
          <article>
            ...
            <button type="button" className="hub-map-link" onClick={onOpenMap}>
              Abrir mapa
            </button>
