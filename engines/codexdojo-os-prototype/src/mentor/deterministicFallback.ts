...
const HELP_LADDER: readonly HelpStep[] = [
  (request) =>
    `Vamos reformular: em "${request.mission.objective}", qual decisao a missao pede que voce tome agora?`,
  (request) =>
    `Recupere o conceito "${request.mission.concepts[0]}". Que criterio dele se conecta ao ponto "${request.interaction.declaredConfusion || 'que ainda parece incerto'}"?`,
  (request) =>
    request.learning.pedagogicalMode === 'developer'
  ...
  (request) =>
    `Pista parcial: compare sua tentativa com uma unica exigencia do objetivo — "${request.mission.objective}". Qual palavra indica o criterio que ainda falta observar?`,
  (request) =>
    request.learning.pedagogicalMode === 'developer'
...
export function answerWithDeterministicFallback(request: MentorRequestV1): MentorResponseV1 {
  const priorMentorTurns = request.interaction.recentTurns.filter(
    (turn) => turn.role === 'mentor',
  ).length
  ...
  if (help === undefined)
    return response(
      request,
      'unavailable',
      'Retome o objetivo da missao e registre uma nova tentativa.',
    )
