...
const STAP_STAGES: readonly MentorStapStage[] = [
  'checking',
  'correcting',
  'complementing',
  'segmenting',
...
export function isMentorRequestV1(value: unknown): value is MentorRequestV1 {
  ...
  if (value.schemaVersion !== MENTOR_SCHEMA_VERSION || !isNonEmptyString(value.requestId, 100))
    return false
  ...
  if (
    ...
    !hasExactKeys(value.mission, ['id', 'trackId', 'objective', 'concepts', 'currentStage'])
  )
    return false
  if (!isNonEmptyString(value.mission.id, 120) || !isNonEmptyString(value.mission.objective, 600))
    return false
  ...
  if (
    ...
    return false
  if (
    value.learning.pedagogicalMode !== 'non-technical' &&
    value.learning.pedagogicalMode !== 'developer'
  )
    return false
  if (
    !['novice', 'advanced_beginner', 'competent', 'proficient', 'expert'].includes(
      String(value.learning.dreyfus),
    )
  )
    return false
  if (
    !['remember', 'understand', 'apply', 'analyze', 'evaluate', 'create'].includes(
      String(value.learning.bloom),
    )
  )
    return false
  if (!isStapStage(value.learning.stapStage)) return false
  if (
    !Number.isInteger(value.learning.stalls) ||
    Number(value.learning.stalls) < 0 ||
    Number(value.learning.stalls) > 20
  )
    return false
  if (
    !isRecord(value.learning.hintQuota) ||
    !hasExactKeys(value.learning.hintQuota, ['used', 'limit'])
  )
    return false
  if (
    !Number.isInteger(value.learning.hintQuota.used) ||
    !Number.isInteger(value.learning.hintQuota.limit)
  )
    return false
  if (Number(value.learning.hintQuota.used) < 0 || Number(value.learning.hintQuota.limit) < 1)
    return false
  if (!isRecord(value.interaction)) return false
  const interactionKeys = Object.keys(value.interaction)
  if (
    !interactionKeys.every((key) =>
      ['question', 'declaredConfusion', 'attemptExcerpt', 'recentTurns'].includes(key),
    )
  )
    return false
  if (
    !interactionKeys.includes('question') ||
    !interactionKeys.includes('declaredConfusion') ||
    !interactionKeys.includes('recentTurns')
  )
    return false
  if (
    !isNonEmptyString(value.interaction.question, 500) ||
    typeof value.interaction.declaredConfusion !== 'string' ||
    value.interaction.declaredConfusion.length > 300
  )
    return false
  if (
    value.interaction.attemptExcerpt !== undefined &&
    !isNonEmptyString(value.interaction.attemptExcerpt, 1_000)
  )
    return false
  if (!Array.isArray(value.interaction.recentTurns) || value.interaction.recentTurns.length > 6)
    return false
  return value.interaction.recentTurns.every(
    (turn) =>
      isRecord(turn) &&
...
export function decodeMentorResponse(raw: unknown, requestId: string): MentorResponseV1 {
  ...
  if (raw.schemaVersion !== MENTOR_SCHEMA_VERSION || raw.requestId !== requestId)
    throw new Error('mentor-response-correlation')
  if (
    !['answered', 'attempt-required', 'quota-exhausted', 'unavailable'].includes(
      String(raw.outcome),
    )
  )
    throw new Error('mentor-response-outcome')
  if (!isNonEmptyString(raw.response, MENTOR_RESPONSE_MAX_LENGTH))
    throw new Error('mentor-response-text')
  if (
    !isRecord(raw.pedagogy) ||
    !hasExactKeys(raw.pedagogy, ['stageBefore', 'stageAfter', 'solutionWithheld'])
  )
    throw new Error('mentor-response-pedagogy')
  if (
    !isStapStage(raw.pedagogy.stageBefore) ||
    !isStapStage(raw.pedagogy.stageAfter) ||
    raw.pedagogy.solutionWithheld !== true
  )
    throw new Error('mentor-response-pedagogy')
  if (
    !isRecord(raw.authority) ||
    !hasExactKeys(raw.authority, ['canonicalStateWritten', 'evidenceCreated', 'masteryEvaluated'])
  )
    throw new Error('mentor-response-authority')
  if (
    raw.authority.canonicalStateWritten !== false ||
    raw.authority.evidenceCreated !== false ||
    raw.authority.masteryEvaluated !== false
  )
    throw new Error('mentor-response-authority')
