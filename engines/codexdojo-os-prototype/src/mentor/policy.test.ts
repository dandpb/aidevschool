...
function request(overrides: Partial<MentorRequestV1['interaction']> = {}, quota = { used: 0, limit: 5 }): MentorRequestV1 {
...
function response(input: MentorRequestV1, overrides: Partial<MentorResponseV1> = {}): MentorResponseV1 {
  return {
    schemaVersion: 1,
...
describe('mentor policy', () => {
  it('requires an attempt and exact confusion before a hint', () => {
    expect(evaluateMentorRequest(request())).toMatchObject({ allowed: false, code: 'attempt-required' })
    expect(evaluateMentorRequest(request({ attemptExcerpt: 'Tentei comparar A e B.' }))).toMatchObject({
      allowed: false,
      code: 'confusion-required',
    })
    expect(evaluateMentorRequest(request({
  ...
  it('stops hints when quota is exhausted', () => {
    ...
    }, { used: 5, limit: 5 }))).toMatchObject({ allowed: false, code: 'quota-exhausted' })
  })

  it('rejects multiple pedagogical transitions and ready-made solutions', () => {
    const input = request({ attemptExcerpt: 'Minha tentativa.', declaredConfusion: 'Travei no criterio.' })
    expect(evaluateMentorResponse(response(input, {
      pedagogy: { stageBefore: 'checking', stageAfter: 'segmenting', solutionWithheld: true },
    }), input)).toMatchObject({ allowed: false, code: 'multiple-transitions' })
    expect(evaluateMentorResponse(response(input, {
      response: 'A resposta correta e B. Copie e cole este resultado.',
    }), input)).toMatchObject({ allowed: false, code: 'solution-revealed' })
  })
