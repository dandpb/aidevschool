...
export function ArchitectureApp({
  ...
  const layers = [
    {
      name: 'Experiência',
      detail: 'Desktop, janelas, navegação e acessibilidade',
      icon: <LayoutGrid />,
      color: 'coral',
    },
    {
      name: 'App SDK',
      detail: 'Manifesto, permissões, armazenamento e eventos',
      icon: <Box />,
      color: 'violet',
    },
    {
      name: 'Motor pedagógico',
      detail: 'Trilhas, domínio, XP, missões e evidências',
      icon: <GraduationCap />,
      color: 'blue',
    },
    {
      name: 'Mentor IA',
      detail: 'Contexto, ferramentas, segurança e avaliações',
      icon: <Bot />,
      color: 'cyan',
    },
    {
      name: 'Runtime',
      detail: 'Processos simulados, filesystem e event bus',
      icon: <Cpu />,
      color: 'slate',
    },
  ]
  return (
    <div className="architecture-app">
      <header>
        ...
        <p>
          Apps publicam ações. A plataforma executa. O motor pedagógico transforma eventos em
          aprendizagem.
        </p>
      </header>
            ...
            onClick={() =>
              onTeach({
                ...
                concepts: [
                  {
                    name: 'Contrato',
                    detail: 'Define entradas, saídas e garantias sem expor detalhes internos.',
                  },
                  {
                    name: 'Responsabilidade',
                    detail: 'Cada camada tem um motivo principal para mudar.',
                  },
                ],
      ...
      <div className="contract-strip">
        <div>
          <GitBranch />
          <span>
            <strong>AppManifest</strong>
            <small>identidade + capacidades</small>
          </span>
        </div>
        <div>
          <Activity />
          <span>
            <strong>LearningEvent</strong>
            <small>ação + conceito + evidência</small>
          </span>
        </div>
        <div>
          <BrainCircuit />
          <span>
            <strong>MentorContext</strong>
            <small>estado + objetivo + limites</small>
          </span>
        </div>
      </div>
    </div>
...
export function SoftwareApp({ onTeach }: { readonly onTeach: (context: LearningContext) => void }) {
  ...
  return (
    <div className="software-app">
      <header>
        <div>
          <span className="section-label">PRÓXIMOS LABORATÓRIOS</span>
          <h1>Expanda o ambiente.</h1>
        </div>
        <div className="catalog-count">
          <strong>{appCatalog.length}</strong>
          <span>apps mapeados</span>
        </div>
      </header>
        ...
        {apps.map((app) => {
          ...
          return (
            <div key={app.name} className="software-row">
              ...
              <span className="software-copy">
                <strong>{app.name}</strong>
                <small>{app.concepts.join(' · ')}</small>
              </span>
              ...
              <button
                ...
                onClick={() => {
                  setInstalled((current) =>
                    isInstalled
                      ? current.filter((item) => item !== app.name)
                      : [...current, app.name],
                  )
