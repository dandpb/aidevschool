...
export function TerminalApp({ onTeach }: { readonly onTeach: (context: LearningContext) => void }) {
  ...
  const execute = (event: FormEvent) => {
    ...
    const responses: Record<string, string[]> = {
      ...
      'learn process': [
        'MICROLIÇÃO: o shell interpretou seu texto.',
        '1. Procurou um comando compatível.',
        '2. Criou um processo com memória e estado.',
        '3. Aguardou a saída e devolveu o controle.',
        'DESAFIO: por que “clear” não apaga arquivos?',
      ],
    }
    if (normalized === 'clear') {
      setLines([])
      ...
      const output = [
        `$ ${command}`,
        ...(responses[normalized] ?? [`comando não encontrado: ${command}`]),
      ]
  ...
  return (
    <div className="terminal-app">
      <div className="terminal-toolbar">
        ...
        <small>bash · modo seguro</small>
      </div>
      <div className="terminal-output" aria-live="polite">
        {lines.map((line) => (
          <div key={line.id} className={line.text.startsWith('MICROLIÇÃO') ? 'lesson-line' : ''}>
            {line.text || '\u00A0'}
          </div>
        ))}
  ...
  return (
    <div className="files-app">
      <aside className="files-sidebar">
        ...
        <button type="button">
          <HardDrive /> Computador
        </button>
        <strong>Aprender</strong>
        <button type="button">
          <GraduationCap /> Meus projetos
        </button>
      </aside>
      <section className="files-main">
        <div className="files-toolbar">
          <button type="button" onClick={() => setPath('/home/daniel')} title="Voltar ao início">
          ...
          <button type="button" aria-label="Buscar arquivos">
            <Search />
          </button>
          <button type="button" aria-label="Menu de arquivos">
            <Menu />
          </button>
        </div>
        <div className="file-grid">
          {items.map((item) => (
            <button
              ...
              onClick={() => onTeach(coreContexts.files)}
            >
              <span className={item.type === 'folder' ? 'file-icon folder' : 'file-icon'}>
                {item.icon}
              </span>
        ...
        <footer>
          {items.length} itens <span>•</span> Clique duas vezes para abrir uma pasta
        </footer>
      </section>
    </div>
  )
