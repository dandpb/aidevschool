 ...
 * L4: dial the hash strength up to fix load skew, then lock in.
...
function renderStatus(node: HTMLElement, state: GameState): void {
  ...
  if (state.phase === "briefing") node.textContent = "Pronto para iniciar."
    ...
    node.textContent = `Caixa ${state.pendingIndex + 1} de ${state.keys.length}: ${key} — clique na prateleira para a qual a chave faz hash.`
    ...
    node.textContent =
      "Chaves quentes concentrando numa prateleira só. Aumente a força do hash e confirme."
}
...
function renderControls(node: HTMLElement, state: GameState, game: GameController): void {
  ...
  if (state.level.id === "L2" || state.level.id === "L3") {
    if (state.crudIndex < state.keys.length) {
      button(node, "get-alive", "viva (valor)", () => game.answerGet(true))
