...
| Game catalog (all 17 `game-*` packages) | `catalog.json` | One entry per implemented game with id, name, and dev port. `game-10-hash-ring/` is the reference, not the only implementation. |
