...
| G8 seed map beyond pilot | **closed (spatial concepts) — open (rules-shaped, 01 only)** | 16 voxelDojo games implemented 2026-07-05 via the threejs-dojo all-18 buildout (games 02, 03, 05, 06, 07, 08, 09, 11, 12, 13, 14, 15, 16, 17, 18 — each full M1–M6: deterministic sim core, Vitest concept proofs, Three.js scene, 4 levels, voxeldojo evidence emit, Playwright smoke) + `game-04-task-queue` (TASK FORGE) added by AID-1901 PR-A1 per CEO decision AID-1859 Opção A: worker-pool dispatch is a multi-stage spatial system (queue → N workers → retry rack → DLQ) that pixel-quest's admit/reject shell would collapse, so the 04 seed moved back to a Shape-B 3D world (plan slice: `../pixelDojo/docs/plans/04_concurrent_task_queue.md`; evaluator = PR-A2, VEE). Only 01 (rate limiter) remains rules-shaped in the sister pixel-quest engine as Shape A encounters (`tokenBucket`/`sequenceFlow`). Pilot `game-10-hash-ring` predates this pass. All voxelDojo games: lint+test+typecheck+build+smoke green (smoke re-verified on a per-wave sample). Each game has a PLAN slice arguing §2 "why 3D". |
