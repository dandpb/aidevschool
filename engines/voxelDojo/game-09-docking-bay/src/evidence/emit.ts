...
const UNIT_ID = "U9-plugin-system"
const PROJECT = "09_plugin_system"
const GAME = "DOCKING BAY"

export interface EvidenceRecord {
  source: "voxeldojo"
  unit_id: "U9-plugin-system"
  project: "09_plugin_system"
  scenario_id: `docking-bay-${LevelId}`
  game: "DOCKING BAY"
  ts: string
  pass: boolean
 ...
 *
 * Note: the hash-ring pilot uses a distinct id `U9-distributed-cache`; the verifier keys on
 * `unit_id` + `project`, so there is no collision.
 */
