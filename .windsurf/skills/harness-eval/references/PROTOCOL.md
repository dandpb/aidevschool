...
- Missing `app/` / `lib/` / `test/` (and similar code-tree, incl. `bin/…`) cites are BROKEN only when mandate language (`load`, `open`, `must`, `required`, …) appears in the same paragraph — bare naming examples are not BROKEN.
- Directory cites (`.agents/skills`) resolve like files.
- `...` / `…` shorthand cites are placeholders.
- Manifest discovery covers subpackages (depth ≤ 3, vendored dirs excluded), so umbrella-repo scripts (`pnpm run lint` inside an engine) are not false BROKEN; leading-dash tokens (`pnpm --filter pkg dev`) are flags, not scripts.
- Unresolved cites fall back to a repo-wide search before being reported (monorepo-relative paths).
- `references/…` cites about a skill that does not exist in this repo are teaching examples, not BROKEN.
