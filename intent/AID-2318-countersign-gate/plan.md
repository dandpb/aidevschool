...
PR context simulated with real resolver + citation citing AID-2318) → board
API key minted and stored as repo secrets (PAPERCLIP_API_URL,
PAPERCLIP_API_TOKEN) → branch + PR (PR body declares the gate will stay red
until the countersign citation is posted — correct per meta-rule) → QA
countersign child issue (fresh-context) → CEO single-writer merge order child
issue citing `Countersign:` in the merge message.
