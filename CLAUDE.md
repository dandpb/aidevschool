...
- **Para o runner OpenClaw:** comece com `python3 -m engines.openclaw --preview`; o estado de
  máquina vem de `learner/pipeline_status.yaml`. Testes: `python3 -m pytest engines/openclaw/tests/`.
- **Para fechar um gate de aprendizado:** o jogo (pixelDojo/voxelDojo) emite evidência; o verificador
  decide: `python3 -m learner.gate --dry-run` → depois `python3 -m learner.substrate` (regenera
