...
def _parse_manifest_file(p: Path, name: str) -> list[str]:
    """Parse one manifest file into command names (empty on unreadable/unknown)."""
    ...
    if name == "package.json":
        ...
        if isinstance(scripts, dict):
            cmds.extend(scripts.keys())
    elif name == "Makefile":
        cmds.extend(_parse_makefile_targets(text))
    elif name in {"Taskfile.yml", "Taskfile.yaml"}:
        cmds.extend(_parse_taskfile_tasks(text))
        ...
        cmds.extend(_parse_rakefile_tasks(text))
    elif name in {"build.gradle", "build.gradle.kts"}:
        cmds.extend(_parse_gradle_tasks(text))
    elif name == "pom.xml":
        cmds.extend(_parse_pom_exec_ids(text))
...
# why: umbrella repos keep runnable scripts in engine subpackages; vendored/dot
# dirs are noise and never define harness commands
_SKIP_DIRS = {
    "node_modules",
    "vendor",
    "dist",
...
def _iter_subpackage_manifests(root: Path, max_depth: int = 3):
    """Yield (path, name) for manifests in subdirectories (depth <= max_depth)."""
    stack = [(root, 0)]
    while stack:
        d, depth = stack.pop()
        try:
...
def load_manifest_commands(root: Path) -> tuple[list[str], list[str]]:
    ...
    for name in MANIFEST_FILES:
        ...
        if name == "artisan":
            ...
            continue
        try:
            commands.extend(_parse_manifest_file(p, name))
        except OSError:
            continue

    # why: subpackage manifests (engine/package.json, nested Makefile) hold the
    # scripts umbrella-repo instructions actually cite
    for p, name in _iter_subpackage_manifests(root):
        try:
            resolved = p.resolve()
        except OSError:
            resolved = p
