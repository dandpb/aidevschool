...
| Collector binding mechanism (Unresolved 1 closed) | Branch (b), test-refreshed: the collector's vocabulary tables become one script-generated sentinel-delimited block inside `dojo-analytics-collector.mjs` (`// GENERATED from engines/shared/teaching-evidence/vocabularies/*.json`), refreshed by `learner/gate/analytics/refresh_vocabularies.mjs`, locked by C1's total JSON↔tables equality; C2 allowlists exactly the derived/loader sites | branch (a) generated `.mjs` module imported from outside the functions dir — no test here can prove the live Netlify bundler follows an import leaving the functions dir, and the OS staged deploy copies byte-identical single files (deploy-pilot-bundle.mjs), so the import would need a copy-list + staging change with live-deploy risk the spike constraints weigh against (AID-961 422 precedent; design Needs-a-spike) |
| TS-side JSON consumption (Contract open point) | New loader module `engines/shared/teaching-evidence/vocabularies.ts` exported as package subpath `./vocabularies` for the surfaces stack (`funnelTelemetry.ts` derives its tables from it); literacyDojo and codexdojo-os-prototype — which carry no dependency edge on `@aidevschool/evidence` — read `vocabularies/{literacy,os}.json` via the established cross-tree relative JSON import with import attributes (`with { type: "json" }`; precedents: literacy `src/domain/evidence.ts` ← `learner/gate/literacy_evidence.schema.json`, OS `src/engines/voxelCatalog.ts` ← `voxelDojo/catalog.json`) | adding `file:../shared/teaching-evidence` dependencies to literacyDojo and the OS prototype — npm install + lockfile churn on two engines for zero behavioral gain while the engines' own precedent already reads cross-tree JSON directly |
